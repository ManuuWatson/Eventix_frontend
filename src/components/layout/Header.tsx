          ...
          <div className="hidden md:flex flex-grow max-w-md mx-4">
            <form onSubmit={handleSearch} className="relative flex w-full">
              ...
              <input
                ...
                className="w-full py-2 pl-10 pr-4 rounded-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                value={searchQuery}
          ...
          <div className="md:hidden mt-4 pb-4">
            <form onSubmit={handleSearch} className="mb-4">
              <input
                type="text"
                placeholder="Search events..."
                className="w-full py-2 px-4 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
            ...
            <nav className="flex flex-col space-y-2">
              <Link to="/" className="text-gray-700 hover:text-indigo-600 py-2">
                Events
              </Link>
              <Link to="/about" className="text-gray-700 hover:text-indigo-600 py-2">
                About
              </Link>
              <Link to="/contact" className="text-gray-700 hover:text-indigo-600 py-2">
                Contact
              </Link>
              ...
              {user ? (
                <>
                  {user.user_type === 'host' && (
                    <Link to="/host" className="text-gray-700 hover:text-indigo-600 py-2">
                      Host Dashboard
                    </Link>
                  )}
                  {user.user_type === 'admin' && (
                    <Link to="/admin" className="text-gray-700 hover:text-indigo-600 py-2">
                      Admin Dashboard
                    </Link>
                  )}
                  <button
